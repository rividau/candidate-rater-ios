...
class Candidates {
    ...
    class func saveUserRatings(candidate: Candidate, callback: (error: NSError?) -> Void) -> Void {
        ...
        if candidate.jsonRatings != nil && candidate.ratingId > 0 {
            urlRequest = Router.UpdateUserRatings(candidate)
        } else {
            urlRequest = Router.CreateUserRatings(candidate)
        }
        NetworkManager.sharedInstance.defaultManager.request(urlRequest).print()
            .responseJSON { (response: Response<AnyObject, NSError>) -> Void in
                response.print()
